...
Citizen.CreateThread(function()
	while true do
		...
		if(var2 ~= 0)then
			drawTxt(0.515, 1.62, 1.0,1.0,0.4, "~w~[~y~" .. tostring(GetStreetNameFromHashKey(var2)) .. "~w~]", 255, 255, 255, 255)
		end

		if(GetStreetNameFromHashKey(var1) and GetNameOfZone(pos.x, pos.y, pos.z))then
			if(zones[GetNameOfZone(pos.x, pos.y, pos.z)] and tostring(GetStreetNameFromHashKey(var1)))then
				drawTxt(0.515, 1.65, 1.0,1.0,0.4, direction .. "~b~ | ~y~" .. tostring(GetStreetNameFromHashKey(var1)) .. " ~w~/ ~y~" .. zones[GetNameOfZone(pos.x, pos.y, pos.z)], 255, 255, 255, 255)
			end
		end
		...
		if(IsPedInAnyVehicle(GetPlayerPed(-1), false))then
			...
			drawTxt(1.407, 0.00, 1.0,1.0,0.7, "~y~" .. math.ceil(speed) .. "", 255, 255, 255, 255)
			drawTxt(1.4, 0.00, 1.0,1.0,0.7, "~b~ mph", 255, 255, 255, 255)
		end
	end
